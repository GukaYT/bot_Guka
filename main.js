...
client.on('message', message =>{ // ивент, когда приходит любое сообщение в чат https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-message
    ...
    if (message.content == '/профиль') { // если пользователь написал "node ." 
...
client.on('messageDelete', message =>{ // ивент, когда удаляется любое сообщение с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
  ...
  client.channels.cache.get("1130201230984097845").send(embed); // айди вашего канала с логами
})
...
client.on('guildMemberAdd', member =>{ // ивент, когда пользователь присоединяется к серверу https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
    ...
    Ты наш \`${client.guilds.get("1124740904540971079").memberCount}\` участник! **`) // айди вашего сервера               !!!!!!!!!!
    ...
    .setColor('RANDOM')
    member.send(embed);
    client.channels.cache.get('1130201230984097845').send(embed2) // айди вашего канала с логами
})
...
async function change() {
    let members = client.guilds.cache.get("1130202921179549696").memberCount // сколько людей на сервере + указать айди своего сервера
    client.channels.cache.get("1124740904540971079").setName(`На сервере: ${members}`); // свой айди войса
}
